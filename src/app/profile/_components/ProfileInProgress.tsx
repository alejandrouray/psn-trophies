...
import { selectInProgressTitles } from './Profile.constants'
import type { ProfileInProgressProps } from './Profile.types'

export function ProfileInProgress({ titles }: ProfileInProgressProps) {
  const inProgress = selectInProgressTitles(titles)
  ...
  return (
    <section aria-labelledby="in-progress-heading">
      ...
      {inProgress.length === 0 ? (
        <p className="text-sm text-muted-foreground leading-relaxed rounded-2xl border border-border bg-card p-6 text-center">
          No games in progress in your synced library. Titles at 0% or 100%
          completion are not listed here.
        </p>
      ) : (
