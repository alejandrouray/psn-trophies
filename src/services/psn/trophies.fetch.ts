...
export async function loadGameTrophyDetails(
  ...
  const titleTrophies = isFulfilled(titleResult)
    ? titleResult.value.trophies
    : []
  const earnedTrophies = isFulfilled(earnedResult)
    ? earnedResult.value.trophies
    : []
